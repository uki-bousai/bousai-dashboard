...
| `seikatsu.html` | **生活支援情報（現在の市民向け公開ページ）**。避難所の受入状況・給水車・仮設トイレ・受け取れる物資を公開 |
| `seikatsu-admin.html` | **生活支援情報の入力画面**。同じ合言葉でログイン。配布物資はタップで報告（在庫を数えない） |
| `seikatsu.json` | 生活支援情報のデータ本体（data.json とは独立） |
| `zaitaku.html` | **在宅避難者 物資支援（公開ページ）**。地区単位の「あと何日分あるか」と補充必要量を表示（個人情報は掲載しない） |
| `zaitaku-admin.html` | **在宅避難者の入力画面**。区長・自治会長・民生委員などが世帯人数と物資残量を登録。日数・補充量は自動計算 |
| `zaitaku-calc.js` | 在宅避難者向けの計算ロジック（残量の正規化 → 残り日数 → 補充必要量 → 地区別集計） |
| `zaitaku-icons.js` | 物資のアイコン（品目IDから線画のSVGを作る。公開ページと入力画面で共有） |
| `home-add.js` | 「ホーム画面に追加」の案内。アプリ内ブラウザで開かれている場合は先にブラウザで開くよう促す |
| `zaitaku.json` | 在宅避難者データ本体（世帯・必要物資・計算基準マスター）。設計は [ZAITAKU.md](ZAITAKU.md) 参照 |
| `zaitaku-demo.json` | 在宅避難者アプリの**検証環境用データ**（仮データ入り）。各ページに `?env=demo` を付けると使われる |
...
- 地図ライブラリは CDN（unpkg.com）、地図タイルは国土地理院（cyberjapandata.gsi.go.jp）から読み込むため、この部分のみインターネット接続が必要です。
...
- 市民向けページはオフライン対応（PWA）。一度開けば圏外でも最後に取得したデータを表示でき、スマホの「ホーム画面に追加」でアプリのように使えます。
